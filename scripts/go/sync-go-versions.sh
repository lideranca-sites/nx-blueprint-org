...
# 
# IMPORTANTE: Este script deve ser personalizado para seu projeto específico.
# Substitua as referências abaixo pelos caminhos e nomes corretos do seu projeto.

readonly SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly WORKSPACE_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"

# CONFIGURAÇÃO: Personalize estas variáveis para seu projeto
...
readonly GO_MODULE_PATH="github.com/your-org/your-project/libs/user-go"  # Personalize aqui
...
if go get "${GO_MODULE_PATH}@v${GO_USER_VERSION}" 2>/dev/null; then
  log_success "Dependência atualizada via go get"
else
  log_info "go get falhou, tentando sed..."
  sed -i.bak "s|require ${GO_MODULE_PATH} v.*|require ${GO_MODULE_PATH} v${GO_USER_VERSION}|g" go.mod
  rm -f go.mod.bak
fi
...
if grep -q "${GO_MODULE_PATH} v${GO_USER_VERSION}" go.mod; then
